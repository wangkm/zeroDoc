    ...
    VARIANT *pVarResult, EXCEPINFO *pExcepInfo, UINT *puArgErr)
{
  ...
  if (DISPATCH_METHOD & wFlags) 
  {
    ...
    invokeMethod = env->GetMethodID(javaSinkClass, "invoke", "(Ljava/lang/String;[Lcom/jacob2/com/Variant;)Lcom/jacob2/com/Variant;");
		if (env->ExceptionOccurred()) { env->ExceptionDescribe(); env->ExceptionClear();}
    ...
    jmethodID getVariantMethod = env->GetMethodID(javaSinkClass, "getVariant", "()Lcom/jacob2/com/Variant;");
		if (env->ExceptionOccurred()) { env->ExceptionDescribe(); env->ExceptionClear();}
    ...
    //Java_com_jacob2_com_Variant_release(env, aVariantObj);
    ...
    for(i=numVariantParams-1,j=0;i>=0;i--,j++) 
    {
		...
		// SF 1689061 change not accepted but put in as comment for later reminder
		//Java_com_jacob2_com_Variant_release(env, arg);
		zeroVariant(env, arg);
		env->DeleteLocalRef(arg);
    }
