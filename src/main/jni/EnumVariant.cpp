...
Java_com_jacob2_com_EnumVariant_Next(JNIEnv* env, jobject _this, jobjectArray vars)
{
  ...
  //
  jclass    clazz = env->FindClass("com/jacob2/com/Variant");
...
Java_com_jacob2_com_EnumVariant_release(JNIEnv* env, jobject _this)
{
  IEnumVARIANT* self = extractEnumVariant(env, _this);
  if(self != NULL)
  {
...
JNIEXPORT void JNICALL
Java_com_jacob2_com_EnumVariant_Reset(JNIEnv* env, jobject _this)
{
  IEnumVARIANT* self = extractEnumVariant(env, _this);
  if(self == NULL)
    return;
...
JNIEXPORT void JNICALL
Java_com_jacob2_com_EnumVariant_Skip(JNIEnv* env, jobject _this, jint count)
{
  IEnumVARIANT* self = extractEnumVariant(env, _this);
  if(self == NULL)
    return;
