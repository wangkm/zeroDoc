  ...
  (JNIEnv *env, jobject _this, jobjectArray a)
{
  ...
  if (vt == VT_VARIANT) {
    ...
    for(int i=0;i<len;i++) {
      ...
      // jacob2 report 1224219 should use unicode and not UTF-8 
  ...
  (JNIEnv *env, jobject _this)
{
  ...
  SafeArrayGetUBound(sa, 1, &ub);
  int num = ub - lb + 1;
  jclass vClass = env->FindClass("com/jacob2/com/Variant");
  ...
  (JNIEnv *env, jobject _this, jint idx)
{
  ...
  SafeArrayGetVartype(psa, &vt);
  // prepare a new return value
  jclass variantClass = env->FindClass("com/jacob2/com/Variant");
  jmethodID variantCons = 
